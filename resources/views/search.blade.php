...
<div class="container mx-auto px-4 py-8">
    ...
    @if(isset($driving_schools))
    <div class="flex">
        <div class="w-1/2">
            <h2 class="text-2xl font-bold mb-4">Driving Schools</h2>
            <ul>
                @foreach($driving_schools as $driving_schools)
                <li class="mb-2">
                    <strong>{{ $driving_school->name }}</strong><br>
                    {{ $driving_schools->address }}
                </li>
                @endforeach
            </ul>
        </div>
...
function initMap() {
    ...
    new google.maps.Marker({
        position: { lat: {{ $driving_schools->latitude }}, lng: {{ $driving_schools->longitude }} },
        map: map,
        title: "{{ $driving_schools->name }}"
    });
    @endforeach
}
